...
class InternetInfiniteList {
  ...
  String getLinkSearch({required ModelSearch m}) {
   ...
   try {
      ...
      if (m.organization.toString() != "null" && m.organization.toString().isNotEmpty) {
        if (data.isNotEmpty) {
      ...
      if (m.categorySlug.toString() != "null" && m.categorySlug.toString().isNotEmpty) {
        if (data.isNotEmpty) {
      ...
      if (m.sessionId.toString() != "null" && m.sessionId.toString().isNotEmpty) {
        if (data.isNotEmpty) {
  ...
  Future<String> searchList({required ModelSearch modelSearch}) async {
    ...
    try {
      log( "${BaseUrl.url}/api/v1/web/products/?${getLinkSearch(m: modelSearch)}");
      response = await dio.get(
